...
class wuziqi:
    def __init__(self, nrow=15, ncol=15, board_width=800, board_height=800, x_margin=100, y_margin=100):
        ...
        self.human_row_col = []
        self.computer_row_col = []
        self.computer_score = np.zeros(shape=(nrow, ncol))
        ...
        for row in range(self.nrow):
            for col in range(self.ncol):
                if self.board[row, col] > 0:
                    row_max = min(row + 1, self.nrow + 1)
                    col_max = min(col + 1, self.ncol + 1)
                    row_min = max(row - 1, 0)
                    col_min = max(col - 1, 0)
    ...
    def minimax(self, current_depth, target_depth, maximize_player_flag, alpha, beta, possible_moves, computer_color):
        ...
        if current_depth == target_depth or self.terminal_state():
    ...
    def get_move(self):
        ...
        for (row, col) in possible_moves:
            ...
            move_score = self.minimax(current_depth, target_depth, False, alpha, beta, possible_moves, self.computer_color)
    ...
    def player_place_button(self, mouse_x=None, mouse_y=None):
        ...
        print("computer move: %d %d" %next_move)
        # computer
        self.board[next_move[0], next_move[1]] = self.computer_color
        move_x, move_y = self.get_xy(next_move[0], next_move[1])
